...
["say", "nfo", "ok", "hey", "no"].forEach((key: CustomLog) => {
	console[key] = (message, ...params) => {
		...
		var styles = [...Array(parts.length)].map((_, i) => {
			return i % 2 == 0
				? `background: ${colors[key][0]}; color: ${colors[key][1]}; font-weight: normal; font-size: 10px; line-height: 10px; padding: 5px 0px`
				: `background: ${colors[key][0]}; color: ${colors[key][2]}; font-weight: bold; font-size: 10px; line-height: 10px; padding: 5px 0px`
        })
